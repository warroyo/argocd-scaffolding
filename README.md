...
deletion is driven by the Applications' finalizers). `destroy-apps` deletes the
...
required; the step runs `apply -refresh-only` first so the short-lived tokens are
current. Requires `kubectl` + `jq`. You can run it alone (e.g. to drain a lab
before rebuilding): `make destroy-apps`.
