...
apiserver trusts VCFA. The audience is an **org-global constant** — the same
VCFA client for every cluster in the org (not keyed by env: env never selects
the identity plane, the org/VCFA does) — so there is no per-cluster/per-tenant
client, no redirect URIs, and nothing to register when a cluster is added.
`components/oidc-auth` is **generated by the infra TF run** (`oidc.tf` +
`templates/oidc-auth.yaml.tftpl`): `issuer` from `var.vcfa_url`,
`certificateAuthority` from a live `tls_certificate` probe of the VCFA endpoint
(a CA roll is `make apply-infra`), and `audiences` from `var.vcfa_oidc_audience`
— the only hand-supplied value. Multi-org (a separate TF run per org — the vcfa
provider is one org per state) re-keys the bundle per org; see `docs/BACKLOG.md`.
...
`oidc-auth` itself is AKO-agnostic — it only touches the apiserver. The
companion `headlamp-config` exposes the UI through Gateway API and defaults its
GatewayClass to `avi-lb` (AKO/AVI is default-on every cluster; Gateway API has
no is-default-class, so this is an explicit default value). A **non-AKO** cluster
running headlamp overrides `className` per-cluster (a `patches:` block targeting
the headlamp `AddonConfig` by label) to a gateway class it does provide — the
override layer is the cluster, same as AKO's own `disable-ako` toggle. Steps:
`docs/GETTING-STARTED.md` "Headlamp SSO".
