...
#### One-time per VCFA org — set one tfvar, the rest is derived

`components/oidc-auth` is **generated by the infra Terraform run**
(`templates/oidc-auth.yaml.tftpl`, rendered by `make apply-infra`) — do not edit
it by hand. The three values are **org-global** (one VCFA org's identity plane,
shared by every cluster it hosts; never per env or per cluster — env doesn't
select the identity plane, the org/VCFA does), and Terraform derives two of them:

- **Issuer** — `${var.vcfa_url}/oidc`, computed in `oidc.tf`. Nothing to set.
- **CA bundle** — pulled live from the VCFA TLS endpoint by a `tls_certificate`
  data source (`oidc.tf`). Nothing to set, and a **CA roll is just
  `make apply-infra`** (it re-probes the endpoint).
- **Audience** — the one value you supply: `var.vcfa_oidc_audience`. It's a
  registered OIDC client id (not a TLS/discovery artifact), so it isn't
  derivable. Read it off a live token once and put it in your tfvars:
  ```
  ./scripts/headlamp-token.sh | cut -d. -f2 | base64 -d 2>/dev/null | jq -r '.aud'
  ```
  (Today this is the VCFA **portal/API** client — see `docs/DECISIONS.md` #21
  on the audience-width trade-off; a prod deployment would mint a dedicated k8s aud.)

So the whole setup is: set `vcfa_oidc_audience`, run `make apply-infra`, commit
the rendered `components/oidc-auth/kustomization.yaml`. The `claimMappings` are
static in the template and stay **byte-identical** to the Concierge
`JWTAuthenticator` in `base/argocd-attach-rbac/config` (`validate.sh` parity
check). A second VCFA org is a separate TF run — the vcfa provider is one org
per state — that re-keys this bundle per org; see `docs/BACKLOG.md`.

#### Per cluster

1. **Opt the cluster in.** In the cluster's `kustomization.yaml` add
   `components/oidc-auth` and `components/headlamp-config` (before
   `cluster-var-injector`), and set `headlamp_hostname` in `cluster-details.yaml`.
   This makes the apiserver trust VCFA tokens (org-global audience) and exposes
   the UI via Gateway.
   **Enabling `oidc-auth` rolls the control plane once** (a single-replica CP is
   ...
   break-glass). `oidc-auth` is AKO-agnostic (pure apiserver config), but
   `headlamp-config` defaults the UI Gateway to the `avi-lb` GatewayClass. On a
   **non-AKO** cluster (`disable-ako`) that class doesn't exist — override it in
   the cluster's `patches:` block:
   ```yaml
   patches:
