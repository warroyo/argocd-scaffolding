...
**The real prerequisite.** Helm-based add-ons need **helm-controller** on the
cluster, which means a **3.7+ cluster class** (`builtin-generic-v3.7.0`, pinned
in `components/envs/{env}`). The 3.7 class is what makes the platform label the
Cluster `addon.addons.kubernetes.vmware.com/helm-controller: automatic` and
install it, supplying the `HelmRepository` CRD and the `vmware-system-helm`
namespace the add-on renders into. On a 3.6 class none of that exists and the
add-on sits at `dependency "helm-controller" not installed` — which is exactly
how this was found.

How VKS distributes registered repos: it creates an internal `helm-repo`
`AddonInstall` in `vmware-system-vks-public` with `clusters: []` and
`crossNamespaceSelection: Allowed`, i.e. every cluster on the Supervisor. That
is the intended fan-out mechanism, not a misconfiguration. It's platform-owned
— `Forbidden` to patch or delete even as Supervisor-admin — and there's no
scoping knob (the CRs are rejected outside `vmware-system-vks-public`,
`AddonRepositoryInstall.spec` has only `addonRepositoryRef`, and the vendor
documents none). Clusters not yet on 3.7 log addon-controller errors for that
ClusterAddon; nothing else about them is affected.

Two undocumented API details, both found by being rejected: the
`AddonRepository` must carry an `addons.kubernetes.vmware.com/package-offerings`
annotation (JSON, mirroring `spec.version` and `spec.addonFilters`) or the
validating webhook refuses it; and registration *does* mint an `AddonRelease`
(`external-secrets.2.8.0`), contrary to an earlier assumption here that the
helm flow skips them — only the naming convention differs.
...
cluster-scoped**, so the audience is an **org-global constant** — one VCFA org's
identity plane, shared by every cluster it hosts, never keyed by env (env doesn't
select the identity plane; the org/VCFA does) — and there is no per-cluster or
per-tenant registration, redirect URI, or secret. `components/oidc-auth` is
generated by the infra TF run (`oidc.tf`): issuer from `var.vcfa_url`, CA from a
live `tls_certificate` probe, audience from `var.vcfa_oidc_audience` (the only
hand-supplied value). A second VCFA org is a separate TF run (the vcfa provider
is one org per state) that re-keys this bundle per org — see `docs/BACKLOG.md`.
This deleted the earlier relying-party scaffold
...
**A2. Forcing the conflict then dropped the platform's labels — once.** With
...
which is why the objects look correct in the cluster while Terraform reports a
mismatch. The applies **do** take effect regardless — the enforcement flips were
live while this error was still firing.

**The manifests deliberately do not declare those labels.** Terraform applies
only the fields this repo owns and lets the platform mutate the rest behind it;
`kubernetes_manifest` already ignores server-side changes to labels and
annotations (`computed_fields` defaults to
`["metadata.annotations", "metadata.labels"]`, verified in provider 2.38.0).
Declaring a platform-namespaced label would mean asserting a value the platform
derives, and fighting it forever if that derivation ever changed.

The expectation is that the eviction is a **one-time migration cost**: the
`before-first-apply` entry only exists until the first server-side apply
displaces it, after which the platform's own controller owns those labels under
its own manager and a forced apply has nothing of its to take. If the
consistency error proves to recur on later runs instead, the field is being
rewritten rather than merged, and the options are (in order) declaring the label
after all, or `lifecycle { ignore_changes }` on the manifest attribute.
