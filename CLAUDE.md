...
| `.kube-backend.config` | `terraform/state-backend` → `local_sensitive_file` in `generate.tf` (kubeconfig for the kubernetes backend; the Makefile points `KUBECONFIG` at it; holds a token; **gitignored**, never commit) |
