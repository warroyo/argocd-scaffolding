...
| `infrastructure/components/oidc-auth/kustomization.yaml` | `terraform/infra` → `templates/oidc-auth.yaml.tftpl` (Headlamp SSO identity bundle; `issuer` derived from `var.vcfa_url`, `certificateAuthority` from a `tls_certificate` probe of the VCFA endpoint, `audiences` from `var.vcfa_oidc_audience`; see `oidc.tf`) |
...
| `var.vcfa_oidc_audience` (+ `var.vcfa_url` + live VCFA cert) + `components/headlamp-config/` | **Headlamp SSO (VCFA identity).** Opt-in per cluster. `components/oidc-auth` is now **TF-generated** (see the generated-files table) — it sets the guest apiserver's structured `AuthenticationConfiguration` to trust VCFA tokens directly. The three values are **org-global** (ONE VCFA org identity plane for every cluster it hosts, so no per-cluster/per-tenant client, no redirect URIs, and **not** keyed by env: env never selects the identity plane, the org/VCFA does): `issuer` is derived from `var.vcfa_url`, `certificateAuthority` from a `tls_certificate` probe (`oidc.tf`), and `audiences` from `var.vcfa_oidc_audience` — the only hand-supplied value (a registered client id, not a TLS/discovery artifact). Multi-org re-keys these per org via a separate TF run — `docs/BACKLOG.md`. `headlamp-config` (hand-authored) exposes the UI via Gateway (`headlamp_hostname` in `cluster-details.yaml`). Users paste the bearer their vcf CLI already mints (`scripts/headlamp-token.sh`) into Headlamp's token field — Headlamp does NO OIDC. Enabling `oidc-auth` **rolls the control plane once** (`validate.sh` warns on single-replica CPs). ClaimMappings stay byte-identical to the Concierge `JWTAuthenticator` in `argocd-attach-rbac` (`validate.sh` parity check) so CLI (cert) and dashboard (bearer) resolve the same RBAC subjects. See `docs/ARCHITECTURE.md` "VCFA identity", `docs/DECISIONS.md` #21. |
