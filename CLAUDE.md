...
| `.kube-backend.env` | `terraform/state-backend` → `local_sensitive_file` in `generate.tf` (KUBE_* creds for the kubernetes backend, sourced by the Makefile; holds a token; **gitignored**, never commit) |
